...
# Format a Julia vector into pgvector's textual input format
function to_pgvector(v::AbstractVector{<:Real})::String
    return "[" * join(string.(Float64.(v)), ",") * "]"
end

module OpenAIClient
using HTTP, JSON3, Random, Statistics
function fake_embed(text::AbstractString, dim::Int=3072)
    seed = UInt32(hash(text) % 0xffffffff); rng = Random.MersenneTwister(seed)
    v = rand(rng, Float32, dim); v ./= sqrt(sum(v.^2) + 1e-6f0); return v
end
function embed(text::AbstractString; model::AbstractString="text-embedding-3-large", dim::Int=3072)
...
HTTP.@register router "POST" "/chaos/rag/index" function(req)
    ...
    for doc in docs
        ...
        r = execute(POOL, "INSERT INTO hd_docs (source,kind,content,meta) VALUES ($1,$2,$3,$4::jsonb) RETURNING id", (src,kind,content,meta)); doc_id = first(r)[1]
        ...
        emb_txt = to_pgvector(emb)
        execute(POOL, "INSERT INTO hd_nodes (id,label,payload,coords,unitary_tag,embedding) VALUES ($1,$2,$3::jsonb,$4,$5,$6::vector) ON CONFLICT (id) DO NOTHING",
                (doc_id, "doc", payload, coords, "identity", emb_txt))
        count += 1
    end
...
HTTP.@register router "POST" "/chaos/rag/query" function(req)
    ...
    emb_txt = to_pgvector(emb)
    rows = execute(POOL, "SELECT id,payload,(embedding <-> $1::vector) AS score FROM hd_nodes ORDER BY embedding <-> $1::vector LIMIT $2", (emb_txt, kv))
